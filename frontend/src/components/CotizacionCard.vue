...
const borrarCotizacion = async (meb) => {
  $q.dialog({
        title: 'Confirmar',
        message: '¿Quieres eliminar la cotización?',
        cancel: true,
        persistent: true
      }).onOk(async() => {
        try {
...
const venderCotiza = async(meb) => {
  
  try {
    successNotify("Cotizacion marcada como vendida");
    useCotizacion.venderCotizacion(meb)
  } catch (error) {
    if (error.errors) {
      return error.errors.forEach((item) => {
        errorNotify(item.msg);
...
<div class="q-pa-md row items-start q-gutter-md">
   ...
   <q-card flat bordered class="my-card bg-grey-1">
           ...
           <div class="text-h6">{{ cotizacion.cliente}} 
            <q-btn color="grey-7" round flat icon="more_vert">
             <q-menu cover auto-close>
               <q-list>
                 <q-item clickable>
                   <q-item-section @click="borrarCotizacion(cotizacion.meb)">Eliminar cotización</q-item-section>
                 </q-item>
     ...
     <q-card-actions>
       <q-btn @click="venderCotiza(cotizacion.meb)" flat>Marcar como vendida</q-btn>
     </q-card-actions>
   </q-card>
</div>
</template>